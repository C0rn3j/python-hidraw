...
_HIDIOCGRAWNAME = lambda length: ioctl_opt.IOC(
    ioctl_opt.IOC_READ, ord('H'), 0x04, length
)
_HIDIOCGRAWPHYS = lambda length: ioctl_opt.IOC(
    ioctl_opt.IOC_READ, ord('H'), 0x05, length
)
_HIDIOCSFEATURE = lambda length: ioctl_opt.IOC(
    ioctl_opt.IOC_WRITE | ioctl_opt.IOC_READ, ord('H'), 0x06, length
)
_HIDIOCGFEATURE = lambda length: ioctl_opt.IOC(
    ioctl_opt.IOC_WRITE | ioctl_opt.IOC_READ, ord('H'), 0x07, length
)
...
class HIDRaw:
    ...
    def _ioctl(
        self, func: int, arg: int | Buffer, mutate_flag: bool = False
    ) -> None:
    ...
    def getRawReportDescriptor(self) -> str:
        """Return a string containing the raw HID report descriptor."""
    ...
    def getFeatureReport(
        self, report_num: int = 0, length: int = 63
    ) -> bytearray:
